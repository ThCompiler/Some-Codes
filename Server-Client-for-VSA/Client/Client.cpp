...
//=================================================================================================================
    ...
    const std::string PathToServer  = "C:\\Users\\Lrnovo\\Source\\Server-Client\\Server-Client\\Debug\\Server.exe"; ///< путь до сервера
    const std::string DirInput      = "toClient\\";                                                                 ///< директория куда отправляются ответы на запросы клиента
...
//-----------------------------------------------------------------------------------------------------------------
//! @ingroup ServerNeed 
//! \brief Отправляет запрос серверу на запуск расчётов
//! 
...
//-----------------------------------------------------------------------------------------------------------------
    ...
    void CaculationRequestsToServer(std::vector<int>* mainArray, std::vector<Buttons::button>* buttonsOfFunction);
}
...
//-----------------------------------------------------------------------------------------------------------------
//! @ingroup ServerNeed
//! функция перевода структуры parameterForResults из json формата
//-----------------------------------------------------------------------------------------------------------------

    void from_json(const json& j, parameterForResults& p) {
        j.at("swaping")         .get_to(p.swaping);
        j.at("comparisons")     .get_to(p.comparisons);
        j.at("numOfElements")   .get_to(p.numOfElements);
    }

//-----------------------------------------------------------------------------------------------------------------
//! @ingroup ServerNeed
//! функция перевода структуры ResultFromServer из json формата
//-----------------------------------------------------------------------------------------------------------------

    void from_json(const json& j, ResultFromServer& p) {
...
int main()
{
    ...
    std::vector<std::string>            listOfAnswerFromServer;                                     // массив названий файлов ответов сервера
    ...
    while (1)
    {
        ...
        listOfAnswerFromServer.clear();
        server::GetListOfInputRequests(&listOfAnswerFromServer);

        if (!listOfAnswerFromServer.empty())
        {
            std::string lastRequests = listOfAnswerFromServer[0];

            txSleep(10);

            json            inputJson;
            string          path            = server::DirInput + lastRequests;
            ...
            if (inputJson["answer"] == 1)
                mainArray = inputJson["newArray"].get<std::vector<int>>();

            if (inputJson["answer"] == 2)
            {
                std::vector<drawing::ResultFromServer> result = inputJson["resultValue"].get<std::vector<drawing::ResultFromServer>>();

                resultValue.clear();

                for (Buttons::button oneButton : buttonsOfFunction)
                    resultValue.push_back({ oneButton.textColor,result[oneButton.id].result });
        ...
        {
            server::CaculationRequestsToServer(&mainArray, &buttonsOfFunction);
        }
...
namespace server {
    ...
    void CaculationRequestsToServer(std::vector<int>* mainArray, std::vector<Buttons::button>* buttonsOfFunction)
    {
            ...
            for (auto each : (*buttonsOfFunction))
                if (each.isPressed)
                    activeFunctions.insert(each.id);

            outputJson["request"]           = 2;
            outputJson["numOfElements"]     = drawing::MaxNumOfElements;
            outputJson["array"]             = (*mainArray);
            outputJson["minNumOfElements"]  = drawing::AccuracyOfDiagram;
            outputJson["activeFunctions"]   = activeFunctions;
            ...
            txMessageBox("Ждём ответа сервера. Он сегодня не в духе так что это на долго.", "Сервисный отдел");
    }
...
//-----------------------------------------------------------------------------------------------------------------
    ...
    void LaunchServer(const std::string* path)
    {
        ...
        ShellExecute(0, "open", path->c_str(), NULL, 0, SW_SHOW); 
    }
...
//-----------------------------------------------------------------------------------------------------------------
    ...
    void GetListOfInputRequests(std::vector<std::string>* listOfInputRequests)
    {
        ...
        std::string path = DirInput + "*.json";

        HANDLE hf = FindFirstFile((LPCSTR)(path).c_str(), &findFileData);

        if (hf != INVALID_HANDLE_VALUE) {

            do {
                listOfInputRequests->push_back(findFileData.cFileName);
