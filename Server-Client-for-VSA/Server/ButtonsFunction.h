...
namespace server {
    ...
    const std::string DirOutput = "toServer\\"; ///< директория куда приходят запросы для сервера
}
...
//!
//! Отправляет запрос серверу на получение нового массива для сортировки
//!
//-----------------------------------------------------------------------------------------------------------------
               ...
               int* MaxNumOfElements = nullptr, std::vector<int>* mainArray = nullptr)
    {
        ...
        (*RandomFilling)            = true;
        outputJson["request"]       = 1;
        outputJson["numOfElements"] = (*MaxNumOfElements);
        outputJson["isRandom"]      = true;
        ...
        MoveFile((LPCSTR)nameFile.c_str(), (LPCSTR)path.c_str());//отправляем созданный запрос серверу
