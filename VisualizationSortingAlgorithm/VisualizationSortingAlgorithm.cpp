...
int main()
{
	...
	while (1)
	{
		...
		if (res == 4) // изменение числа элементов массива
		{
			...
			if (input > int(1e6))
				txMessageBox("Слишком много ввёл. Не дождёшься результата. Максимально можешь ввести до 10^6.",
					"Тех. поддержка");
			else
			{
...
namespace drawing
{
		...
		if (txMouseButtons() == 1)// проверяем нажата ли левая кнока мыши
		{
			...
			for (int i = 0; i < (*simpleButtons).size(); i++) // проверяем обычные кнопки
					...
					&& In(y, (*simpleButtons)[i].y0, (*simpleButtons)[i].y1))
				{
					...
					{
						(*simpleButtons)[i].ClickButton(BackColor);
					}
					else // если имеет
																...
																std::vector<button>* swapButtons, bool defaultDiagram)
	{
		...
		for (button oneButton : (*simpleButtons))
			oneButton.DrawButton(BackColor);
